...
export function ProjectPipeline({ initialProjectId }: ProjectPipelineProps) {
  ...
  const loadProjectIntoGenerator = (projectId: string, fromConcept: boolean = false) => {
    ...
    console.log(`[Generator] Loading project: ${project.title}`);
    ...
    // If project has saved generator state, restore it completely
  ...
  if (setupPhase !== 'ready' || !pipelineState) {
    return (
      <div className="max-w-4xl mx-auto space-y-6">
            ...
            <SelectContent>
              {Object.entries(LANGUAGE_NAMES).map(([code, name]) => (
                <SelectItem key={code} value={code}>
                  <span className="flex items-center gap-2">
                    <span className="text-lg">{LANGUAGE_FLAGS[code as Language]}</span>
                    {name}
                  </span>
                </SelectItem>
              ))}
        ...
        {/* Safety Reset Button - Visible when things get stuck */}
        {(isGenerating || isImproving || isLoadingSuggestions) && (
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-500 to-yellow-500 text-white rounded-full text-sm font-medium mb-4">
            <Brain className="h-4 w-4" />
            {t('aiProjectAssistant')}
          </div>
        ...
        {/* Progress Indicator - Documents step removed, knowledge pool is now in the main generator */}
        <div className="flex justify-center gap-2 mb-8 flex-wrap">
          {['idea', 'consortium', 'workpackages'].map((phase, idx) => {
            const phases = ['idea', 'consortium', 'workpackages'];
            // Map old 'documents' phase to 'workpackages' for backwards compatibility
            const effectivePhase = setupPhase === 'documents' ? 'workpackages' : setupPhase;
            const currentIdx = phases.indexOf(effectivePhase);
            const isComplete = idx < currentIdx;
            const isCurrent = phase === effectivePhase;

            return (
              <React.Fragment key={phase}>
                <button
                  onClick={() => idx <= currentIdx && setSetupPhase(phase as typeof setupPhase)}
                ...
                >
                  {isComplete ? (
                    ...
                    <span className={`w-5 h-5 rounded-full border-2 flex items-center justify-center text-xs font-bold ${isCurrent ? 'border-white' : 'border-current'
                      }`}>
                      {idx + 1}
                    </span>
                  )}
                  <span className="font-medium hidden sm:inline">
                    {phase === 'idea' && t('projectIdea')}
                    {phase === 'consortium' && t('consortium')}
                    {phase === 'workpackages' && (language === 'de' ? 'Arbeitspakete' : 'Work Packages')}
                  </span>
                </button>
                {idx < 2 && <ChevronRight className="h-5 w-5 text-gray-300 self-center" />}
              </React.Fragment>
            );
          })}
        </div>
            ...
            <CardContent className="space-y-6 pt-6">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {ideaDescription.length} / 500 {language === 'de' ? 'Zeichen' : 'characters'}
                </p>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {targetGroupOptions.map((tg) => (
                    <button
                      key={tg.value}
                      onClick={() => {
                        if (targetGroups.includes(tg.value)) {
                          setTargetGroups(targetGroups.filter((t) => t !== tg.value));
                        } else {
                          setTargetGroups([...targetGroups, tg.value]);
                        }
                      }}
                      className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${targetGroups.includes(tg.value)
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select value={actionType} onValueChange={(v) => setActionType(v as typeof actionType)}>
                    ...
                    <SelectContent>
                      <SelectItem value="KA220">KA220 (Kooperationspartnerschaften)</SelectItem>
                      <SelectItem value="KA210">KA210 (Kleinere Partnerschaften)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  ...
                  <Select value={sector} onValueChange={(v) => setSector(v as typeof sector)}>
                    ...
                    <SelectContent>
                      <SelectItem value="ADU">{t('adultEducation')} (ADU)</SelectItem>
                      <SelectItem value="VET">{t('vocationalTraining')} (VET)</SelectItem>
                      <SelectItem value="SCH">{t('schoolEducation')} (SCH)</SelectItem>
                      <SelectItem value="YOU">{t('youth')} (YOU)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-2 text-blue-900">
                      {t('numberOfWorkPackages')}
                    </label>
                    <Select value={wpCount.toString()} onValueChange={(v) => setWpCount(Number(v))}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="3">{t('wp3Minimal')}</SelectItem>
                        <SelectItem value="4">{t('wp4')}</SelectItem>
                        <SelectItem value="5">{t('wp5Standard')}</SelectItem>
                        <SelectItem value="6">{t('wp6')}</SelectItem>
                        <SelectItem value="7">{t('wp7Complex')}</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
              ...
              <Button
                onClick={() => setSetupPhase('consortium')}
                disabled={!ideaDescription || !mainObjective || targetGroups.length === 0}
                className="w-full py-6 text-lg bg-[#003399] hover:bg-[#002266] mt-6"
              >
                {t('continueToConsortium')}
        ...
        {/* Step 2: Consortium */}
        {
          setupPhase === 'consortium' && (
            <Card className="border-2">
              <CardHeader className="bg-gradient-to-r from-blue-50 to-purple-50 border-b">
                <CardTitle className="flex items-center gap-2">
                  <div className="p-2 bg-[#003399] rounded-lg text-white">
                    <Users className="h-5 w-5" />
                  </div>
                  {t('stepBuildConsortium')}
                </CardTitle>
                <CardDescription>
                  {language === 'de'
                    ? `Wähle Partner aus deinem CRM oder füge neue hinzu (min. ${minPartners} für ${actionType})`
                    : `Select partners from your CRM or add new ones (min. ${minPartners} for ${actionType})`}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6 pt-6">
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <div className="flex items-center justify-between p-4 bg-gray-50">
                      <div
                        ...
                        onClick={() => setShowCrmPartners(!showCrmPartners)}
                      >
                        <Database className="h-5 w-5 text-[#003399]" />
                        <span className="font-medium">
                          {t('partnersFromCRM')}
                        </span>
                        <Badge variant="secondary">{crmPartners.length}</Badge>
                        {showCrmPartners ? <ChevronDown className="h-5 w-5 ml-2" /> : <ChevronRight className="h-5 w-5 ml-2" />}
                      </div>
                    ...
                    {showCrmPartners && (
                      <div className="p-4 space-y-2 max-h-80 overflow-y-auto">
                        ...
                        {consortiumAnalysis && (
                          <div className="mb-4 p-3 bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg border border-purple-200">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <Badge className="bg-purple-500">{consortiumAnalysis.overallScore}/100</Badge>
                            </div>
                            <p className="text-sm text-purple-700 mb-2">{consortiumAnalysis.coverageAssessment}</p>
                            {consortiumAnalysis.missingExpertise?.length > 0 && (
                              <p className="text-xs text-purple-600">
                                <span className="font-medium">{t('missing')}</span> {consortiumAnalysis.missingExpertise.join(', ')}
                              </p>
                            )}
                          </div>
                        )}
                        ...
                        {crmPartners.map((partner) => {
                          const isSelected = selectedCrmPartners.includes(partner.id);
                          const isSuggested = suggestedPartners.includes(partner.id);
                          const suggestionDetail = partnerSuggestionDetails[partner.id];
                          ...
                          return (
                            <div
                              ...
                              onClick={() => {
                                if (isSelected) {
                                  setSelectedCrmPartners(selectedCrmPartners.filter(id => id !== partner.id));
                                } else {
                                  setSelectedCrmPartners([...selectedCrmPartners, partner.id]);
                                }
                              }}
                            ...
                            >
                              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 mt-0.5 ${isSelected ? 'bg-white border-white' : 'border-gray-300'
                                }`}>
                                {isSelected && <Check className="h-3 w-3 text-[#003399]" />}
                              </div>
                                ...
                                <div className="font-medium flex items-center gap-2 flex-wrap">
                                  ...
                                  {isSuggested && !isSelected && (
                                    <span className="text-xs bg-yellow-200 text-yellow-800 px-2 py-0.5 rounded-full flex items-center gap-1">
                                      ...
                                      {suggestionDetail?.matchScore ? `${suggestionDetail.matchScore}%` : t('recommended')}
                                    </span>
                                  )}
                                  {suggestionDetail?.suggestedRole && (
                                    <Badge variant="outline" className={`text-xs ${isSelected ? 'border-white text-white' : 'border-blue-300 text-blue-700'}`}>
                                      {suggestionDetail.suggestedRole === 'LEAD' ? '👑 Lead' : suggestionDetail.suggestedRole.replace('_', ' ')}
                                    </Badge>
                                  )}
                                </div>
                                <div className={`text-sm ${isSelected ? 'text-blue-100' : 'text-gray-500'}`}>
                                  {partner.country} · {partner.organizationType}
                                </div>
                                {isSuggested && suggestionDetail?.reason && !isSelected && (
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      placeholder="https://www.organisation.eu"
                      className="flex-1"
                    />
                    <Button onClick={handleAddPartnerFromUrl} disabled={isLoadingPartner || !newPartnerUrl}>
                      {isLoadingPartner ? (
                ...
                {manualPartners.length > 0 && (
                  <div>
                    <label className="block text-sm font-medium mb-2">
                      {t('newlyAddedPartners')}
                    </label>
                    <div className="space-y-2">
                      {manualPartners.map((partner, idx) => (
                        <div
                          key={partner.id}
                        ...
                        >
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => setManualPartners(manualPartners.filter((_, i) => i !== idx))}
                          >
                ...
                {/* Partner Role Assignments - show when partners are selected */}
                {selectedCrmPartners.length > 0 && (
                  <div className="border rounded-lg overflow-hidden">
                    <div className="p-4 bg-gradient-to-r from-purple-50 to-indigo-50 border-b">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-purple-800">
                            {language === 'de' ? 'Rollen-Zuweisung' : 'Role Assignment'}
                          </span>
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={analyzeConsortiumRoles}
                      ...
                      {!ideaDescription && (
                        <p className="text-xs text-purple-600 mt-2">
                          💡 {language === 'de' ? 'Gib zuerst eine Projektidee ein für intelligente Rollen-Vorschläge' : 'Enter a project idea first for intelligent role suggestions'}
                        </p>
                      )}
                    </div>

                    {/* Detailed Consortium Analysis */}
                    {detailedConsortiumAnalysis && (
                      <div className="p-4 bg-white border-b space-y-3">
                        <div className="flex items-center gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-sm font-medium">Konsortium-Score</span>
                              <Badge className={detailedConsortiumAnalysis.overallScore >= 70 ? 'bg-green-500' : detailedConsortiumAnalysis.overallScore >= 50 ? 'bg-yellow-500' : 'bg-red-500'}>
                                {detailedConsortiumAnalysis.overallScore}/100
                              </Badge>
                            </div>
                            <Progress value={detailedConsortiumAnalysis.overallScore} className="h-2" />
                          </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div className="p-2 bg-blue-50 rounded">
                            <span className="text-blue-700 font-medium">🌍 Geographie:</span>
                            <span className="ml-2 text-blue-600">{detailedConsortiumAnalysis.geographicCoverage.countries.length} Länder</span>
                          </div>
                          <div className="p-2 bg-green-50 rounded">
                            <span className="text-green-700 font-medium">✓ Expertise:</span>
                            <span className="ml-2 text-green-600">{detailedConsortiumAnalysis.expertiseCoverage.score}%</span>
                          </div>
                        </div>

                        {detailedConsortiumAnalysis.warnings.length > 0 && (
                          <div className="p-2 bg-red-50 rounded border border-red-200">
                            {detailedConsortiumAnalysis.warnings.map((w, i) => (
                              <p key={i} className="text-sm text-red-700">⚠️ {w}</p>
                            ))}
                          </div>
                        )}

                        {detailedConsortiumAnalysis.recommendations.length > 0 && (
                          <div className="p-2 bg-blue-50 rounded border border-blue-200">
                            {detailedConsortiumAnalysis.recommendations.slice(0, 2).map((r, i) => (
                              <p key={i} className="text-sm text-blue-700">💡 {r}</p>
                            ))}
                          </div>
                        )}
                      </div>
                    ...
                    {/* Partner Skills Matrix for Setup Phase */}
                    {crmPartners.filter(p => selectedCrmPartners.includes(p.id)).length > 0 && (
                      <div className="p-4 border-b border-gray-100 bg-white">
                        <PartnerSkillsMatrix
                          partners={crmPartners
                            .filter(p => selectedCrmPartners.includes(p.id))
                            .map(p => convertToConsortiumPartner(p, partnerRoleAssignments[p.id]))}
                          language={language}
                        />
                      </div>
                    )}
                    ...
                    <div className="p-4 space-y-3">
                      {crmPartners
                        ...
                        .map((partner) => {
                          ...
                          const isCoordinator = coordinatorId === partner.id;

                          return (
                            <div
                              key={partner.id}
                              className={`p-4 rounded-lg border-2 transition-all ${isCoordinator
                            ...
                            >
                              <div className="flex items-start justify-between gap-4">
                                <div className="flex-1">
                                  ...
                                  {assignment && (
                                    <div className="mt-2 text-sm">
                                      <p className="text-gray-600">{assignment.roleRationale}</p>
                                ...
                                <div className="flex flex-col gap-2">
                                  {/* Role Selector */}
                                  <Select
                                    value={assignment?.suggestedRole || 'PARTNER'}
                                    onValueChange={(value) => handleChangePartnerRole(partner.id, value as PartnerRoleAssignment['suggestedRole'])}
                                  >
                                    <SelectTrigger className="w-40">
                                      <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                      {CONSORTIUM_ROLES.map((role) => (
                                        <SelectItem key={role.role} value={role.role}>
                                          {role.role === 'COORDINATOR' ? '👑 ' : ''}{role.labelDE}
                                        </SelectItem>
                                      ))}
                                    </SelectContent>
                                  </Select>
                                  ...
                                  {!isCoordinator && (
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      onClick={() => handleSetCoordinator(partner.id)}
                                      className="text-xs"
                                    >
                                      Als Koordinator
                                    </Button>
                              ...
                              {/* WP Lead Suggestions */}
                              {assignment?.suggestedWPLeads && assignment.suggestedWPLeads.length > 0 && (
                                <div className="mt-3 pt-3 border-t border-gray-100">
                                  <span className="text-xs text-gray-500">
                                    Empfohlene WP-Leitung:
                                  </span>
                                  <div className="flex gap-1 mt-1">
                                    {assignment.suggestedWPLeads.map((wpNum) => (
                                      <Badge key={wpNum} variant="outline" className="text-xs">
                                        WP{wpNum}
                                      </Badge>
                                    ))}
                                  </div>
                ...
                {/* Selected Count */}
                <div className={`p-4 rounded-lg flex items-center justify-between ${hasEnoughPartners ? 'bg-green-50 border border-green-200' : 'bg-orange-50 border border-orange-200'
                  }`}>
                  <div className="flex items-center gap-2">
                    {hasEnoughPartners ? (
                    ...
                    <span className={hasEnoughPartners ? 'text-green-700' : 'text-orange-700'}>
                      {allPartners.length} / {minPartners}+ {t('partnersSelected')}
                      {coordinatorId && (
                        <span className="ml-2 text-sm">
                          (👑 {crmPartners.find(p => p.id === coordinatorId)?.organizationName || 'Koordinator gewählt'})
                        </span>
                      )}
                    </span>
                  </div>
                  {allPartners.length > 0 && (
                    <div className="flex gap-1">
                      {allPartners.slice(0, 5).map((p, idx) => (
                        <div
                          key={p.id}
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${p.isLead ? 'bg-yellow-500 text-yellow-900' : 'bg-[#003399] text-white'}`}
                          title={p.name}
                        >
                          {p.country}
                        </div>
                ...
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => setSetupPhase('idea')} className="flex-1">
                    {language === 'de' ? 'Zurück' : 'Back'}
                  </Button>
                  <Button
                    onClick={() => setSetupPhase('workpackages')}
                    disabled={!hasEnoughPartners}
                    className="flex-1 bg-[#003399] hover:bg-[#002266]"
                  >
                    {language === 'de' ? 'Weiter zu Arbeitspaketen' : 'Continue to Work Packages'}
        ...
        {/* Step 3: Work Packages Configuration (Documents step removed - now handled by Knowledge Pool in generator) */}
        {
          setupPhase === 'workpackages' && (
            <Card className="border-2">
              <CardHeader className="bg-gradient-to-r from-purple-50 to-indigo-50 border-b">
                <CardTitle className="flex items-center gap-2">
                  ...
                  {language === 'de' ? 'Arbeitspakete konfigurieren' : 'Configure Work Packages'}
                </CardTitle>
                <CardDescription>
                  {language === 'de'
                    ? 'Definieren Sie die Struktur Ihrer Arbeitspakete basierend auf den Erasmus+ Richtlinien. Wählen Sie Lead-Partner, Aktivitäten und Deliverables für jedes WP.'
                    : 'Define your work package structure based on Erasmus+ guidelines. Select lead partners, activities and deliverables for each WP.'}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6 pt-6">
                {/* WP Budget Configurator */}
                {allPartners.length > 0 ? (
                  <WPBudgetConfigurator
                    consortium={allPartners}
                    coordinatorId={coordinatorId}
                    totalLumpSum={totalBudget}
                    language={language}
                    onConfigurationChange={setWpBudgetConfigs}
                    initialConfigs={wpBudgetConfigs}
                  />
                ) : (
                  <div className="text-center py-8 text-gray-500">
                    <Users className="h-12 w-12 mx-auto mb-4 text-gray-300" />
                    <p>{language === 'de' ? 'Bitte fügen Sie zuerst Partner im Konsortium-Schritt hinzu.' : 'Please add partners in the Consortium step first.'}</p>
                  </div>
                )}

                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => setSetupPhase('consortium')} className="flex-1">
                    {language === 'de' ? 'Zurück' : 'Back'}
                  </Button>
                  <Button
                    onClick={handleStartPipeline}
                    disabled={wpBudgetConfigs.length === 0 || wpBudgetConfigs.reduce((sum, wp) => sum + wp.totalBudget, 0) !== totalBudget}
